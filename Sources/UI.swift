...
enum UI {
    ...
    private static func dress(_ a: NSAlert) {
        ...
        // An alert has no title bar, so if anything ever does end up on top of one
        // there is no way to drag it clear. Let it be dragged by its face.
        a.window.isMovableByWindowBackground = true
        // A menu bar app is often not the active app when a dialog goes up.
