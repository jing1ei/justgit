...
final class MainController: NSObject, NSWindowDelegate {
    ...
    /// A menu item's action fires while `popUp` is still tracking the menu. An
    /// NSAlert opened from inside that tracking session never becomes key: it
    /// waits for an answer nobody can give it, and the panel looks frozen. The
    /// default run loop mode is not run during menu tracking, so scheduling the
    /// real work there holds it back until the menu has closed.
